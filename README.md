...
**Private access:** Firebase uses HTTP Basic authentication over HTTPS, with username `research` and a random high-entropy password held in Google Secret Manager (`disclosure-lab-access-password`). The server checks credentials before serving **every** page, JavaScript asset and data file. All application responses are `private, no-store`; there is no unauthenticated static-asset/CDN bypass. Startup fails closed if the password is missing or shorter than 32 characters. This is single-owner password protection, not Google sign-in, per-user permissions or an audit system; do not distribute the password or treat it as multi-user authorization.
